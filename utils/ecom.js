...
const getRandomId = () =>
  [...Array(10)].map((i) => (~~(Math.random() * 36)).toString(36)).join(""); // Return a random id
...
const selectors = {
  ...
  flipkart: {
    ...
    availability: "div._1AtVbE > div._16FRp0",
  },
  snapdeal: {
    title:
      "#productOverview > div.col-xs-14.right-card-zoom.reset-padding > div > div.pdp-fash-topcenter-inner.layout > div.row > div.col-xs-18 > h1",
    price1:
...
const getProductDetails = async (url, merchant) => {
  try {
    ...
    const availability = $(selector.availability).text()
    if (!title || !price) {
      return { ok: false };
    }
    return { ok: true, title, price, image, link: commonUrl, availability };
  } catch (e) {
    return { ok: false };
  }
};

module.exports = { isUrl, getRandomId, getProductDetails, productCommonUrl };
